...
export default Component.extend(EmberValidator, Api, {
  ...
  actions: {
    next() {
      ...
      if (model.get('form_status') !== 'completed') {
        data = {
          ...
          ema_bpd: formatToServer(`${model.get('form.nurse_blood_pressure_date_formatted')} ${model.get('form.nurse_blood_pressure_time')}`),
          rca_rpo: formatToServer(`${model.get('form.nurse_rtq_date')} ${model.get('form.nurse_rtq_time')}`),

          d_e: model.get('form.ethnicity'),
          d_ssn: model.get('form.ssn'),
          customer_id: this.get('form.customer_id'),
          assessor_id: this.get('session.role_nurse') ? model.get('form.assessor_id') : undefined,
          agency_staff_id: this.get('form.agency_staff_id'),
          agency_admin_id: this.get('form.agency_admin_id'),
          ra_scr: this.get('form.staff_completing_rtq'),
